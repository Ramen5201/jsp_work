...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * => 같은 파일명이 존재할경우 덮어씌워질 수 있고, 파일명에 한글/특수문자/띄워쓰기 포함 될 경우 서버에 따라서 문제발생
			 * 
			 * 기본적으로 파일명이 안겹치도록 수정작업해주는 객체 있다.
			 * => DefaultFileRenamePolicy(cos에서 제공)
			 * => 내부적으로 해당 클래스에서 rename()메소드가 실행되면서 파일명 수정된 후 업로드
			 * rename(원본파일){
			 * 		기존에 동일한 파일명이 존재할경우
			 * 		파일명뒤에 카운팅된 숫자를 붙여줌
			 ...
			 * 		return 수정파일 
			 *  }
			 *  위와 같은 방식이기 때문에 사용하지 않는다.
			 *  나만의 방식대로 절대 안겹치도록 rename 할수 있게 FileRenmaePolicy클래스 만들기
			 *  com.kh.common.MyFileRenamePolicy
			 */
			
			// MultipartRequest multiRequest = new MultipartRequest(request, 저장시킬폴더의 경로, 용량제한, 인코딩값, new DefaultFileRenmaePolicy()
			...
			// 4.서비스요청
			int result = new BoardSerivce().insertBoard(b, at);
